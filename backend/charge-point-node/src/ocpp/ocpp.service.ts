...
export class OcppService implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap() {
        ...
        this.MyOcppServer.on('connection', (client: OcppClientConnection) => {
            ...
            this.logger.log(`WebSocket connection established with client: ${client.getCpId()}`);
    
            // Optional: Log the timestamp of the connection
            const connectionTime = new Date();
            this.logger.log(`Connection established at: ${connectionTime.toISOString()}`);
    
            // Handling messages from the client (if needed)
          
            // Handling closure of connection
